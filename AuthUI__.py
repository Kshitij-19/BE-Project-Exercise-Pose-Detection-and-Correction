...
def register():
    ...
    name_entry.pack()
    phone_lable = Label(register_screen, text="Phoneno * ")
    phone_lable.pack()
...
def register_user():
    ...
    try:
      ...
      #  db.child("users").child(user['idToken']).set(data)
        u = User(email , password_ , nname , mobno)
        print(u.to_dict())
        ...
        print(e)
        print("Email Already Exists")
        Label(register_screen, text="Email Already Exists", fg="red", font=("calibri", 11)).pack()
...
def process_login(emailVerified):
    ...
    if emailVerified:
        ...
        login_success_screen.geometry("150x100")
        Label(login_success_screen, text="Email is not verified").pack()
